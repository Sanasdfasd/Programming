//
// Created by Sandeep on 3/11/2018.
//
    ...
    for (int k = 0; k <adj[ply1].size() ; ++k) {
      ...
      if(adj[ply1][k].first==ply2) {
        ...
        for (int i = 0; i < adj[ply2].size(); ++i) {
          ...
          if(ply2_move==ply1 && (int)ply2_move_char>=(int)ply1_move_char){
            dp[ply1][ply2]=1;
            return;
          }
          if (((int) ply2_move_char) >= ((int) ply1_move_char)) {
            tr = 1;
            if (dp[ply1_move][ply2_move] == 1){ cnt=1;break;}
          }
        }
        if (tr == 0 ||cnt==0) {
          dp[ply1][ply2]=0;
...
int main(){
  ...
  cout<<"the toplogical order is "<<endl;
  ...
  for (int i =order.size()-1; i >=0; --i) {
    for (int j = i; j <order.size() ; ++j) {
      ...
      if(ply1!=ply2)
        Pos(ply2,ply1);
      cout<<" the dp value is ply1 and ply2:  "<<ply1<<" "<<ply2<<" "<<dp[ply1][ply2]<<"  "<<dp[ply2][ply1]<<endl;
    }
  }
  for (int i = 0; i <n ; ++i) {
    for (int j = 0; j <n; ++j) {
